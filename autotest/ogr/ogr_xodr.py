...
def check_feat_def_reference_line(layer):
    assert layer.GetGeomType() == ogr.wkbLineString25D, "bad layer geometry type for ReferenceLine"
...
def check_feat_def_lane_border(layer):
    assert layer.GetGeomType() == ogr.wkbLineString25D, "bad layer geometry type for LaneBorder"
...
def check_feat_def_road_mark(layer, dissolve_tin: bool):
    if not dissolve_tin:
        assert layer.GetGeomType() == ogr.wkbTINZ, "bad layer geometry type for RoadMark"
    else:
        assert layer.GetGeomType() == ogr.wkbPolygon25D, "bad layer geometry type for dissolved RoadMark"
...
def check_feat_def_lane(layer, dissolve_tin: bool):
    if not dissolve_tin:
        ...
        assert layer.GetGeomType() == ogr.wkbPolygon25D, "bad layer geometry type for dissolved Lane"
...
def check_feat_def_road_signal(layer, dissolve_tin: bool):
    if not dissolve_tin:
        assert layer.GetGeomType() == ogr.wkbTINZ, "bad layer geometry type for RoadSignal"
    else:
        assert layer.GetGeomType() == ogr.wkbPoint25D, "bad layer geometry type for dissolved RoadSignal"
...
def ogr_xodr_check_reference_line_geometry_eps(lyr, eps: float):
    ...
    if eps == 1.0:
        assert wkt == "LINESTRING (618251.572934302 5809506.96459625 102.378603962182,618254.944363001 5809506.95481165 102.371268481462,618258.290734177 5809506.56065761 102.363999939623)", "wrong geometry created for ReferenceLine with EPS " + str(eps)
    elif eps == 0.1:
        assert wkt == "LINESTRING (618251.572934302 5809506.96459625 102.378603962182,618254.944363001 5809506.95481165 102.371268481462,618257.937110798 5809506.62607284 102.364759846201,618258.290734177 5809506.56065761 102.363999939623)", "wrong geometry created for ReferenceLine with EPS " + str(eps)
